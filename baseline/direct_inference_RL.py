...
def build_prompt(question: str, si: str, use_si: bool = True) -> str:
    ...
    if use_si and si.strip():
        ...
        parts.append(si)
    parts.append(
        # "Put ALL your reasoning ONLY inside <think>...</think>.\n"
...
def is_match(gold_list, pred_list):
    """
    gold_list / pred_list 都是已经 normalize 过的小写字符串列表
    判对规则：
    1. gold 多项：gold 的每一项都能在 pred_list 里找到（精确或子串）→ ✅
    2. gold 单项：只要有一项 pred 跟它互为子串之一 → ✅
    """
    # 先处理多项 gold 的情况
    if len(gold_list) > 1:
        for g in gold_list:
            ...
            if not ok:
                return False
        return True  # 所有 gold 都能在 pred 中找到
